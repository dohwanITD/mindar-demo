...
AFRAME.registerComponent('step-ctrl', {
  ...
  init() {
    ...
    // const account = document.getElementById('type-account')
    // const name = document.getElementById('type-name')
    // const money = document.getElementById('type-money')
    // account.pause()
    // money.pause()
    // name.pause()
    ...
    const captionList = [
      ...
      '받는 분 계좌번호와\n금액을 입력해보겠습니다!',  // ~입력하고
      '받는 분 통장 표시와\n자필성명을 입력하신 후\n작성완료 버튼을\n눌러보겠습니다!',  // <<<<<<<<<<< 입력하신후 - tts수정
      '입금 신청서 미리작성이\n완료되었습니다!\n이제 미리작성서비스를 직접 이용해보세요!',  // <<<<<<<<<<<< 멘트 수정, tts추가
    ]
    ...
    // 애니메이션
    // const animationList = ['Jump', 'Yes', 'No', 'Wave', 'Punch', 'ThumbsUp']
    ...
    // account.setAttribute('visible', false)
    ...
    const userClickHandler = () => {
      ...
      sound = new window.Howl({
        src: [require(`./assets/audios/tts_0${stepIdx}.mp3`)],
      })
      sound.play()
      ...
      if (stepIdx === 0) {
        ...
        //     account.setAttribute('visible', true)
      ...
      // clickCursor.position.x = tooltipBox.position.x + tooltipBox.width/2 - 0.215
